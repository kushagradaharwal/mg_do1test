...
class Excellence_Ajax_IndexController extends Mage_Checkout_CartController
{
   ...
   $cartitem =  Mage::helper("checkout/cart")->getSummaryCount();
   $cart_it = Mage::getModel("checkout/cart")->getQuote()->getAllitems();
   //echo '<pre>';print_r($cart_it->getData());
   $prod_id = array();
   foreach ($cart_it as $item) {
    $prod_id[] = $item->getProduct()->getId();
   }
   $pro_idcheck = in_array(170,$prod_id);
   if($pro_idcheck)
   { 
    //echo "yes id in cart"; 
    if($cartitem < 5)
