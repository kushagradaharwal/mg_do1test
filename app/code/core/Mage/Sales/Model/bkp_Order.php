...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    public function sendNewOrderEmail()
    {
        ...
        $emailInfo1 = Mage::getModel('core/email_info');
		$emailInfo1->addTo("kushagra.daharwal@hotmail.com", "ram");
       
        $mailer1->addEmailInfo($emailInfo1);
        ...
        //$mailer1->send();
          //new code end
			...
			{
			$filearray[] = $collection1['filename'];
			}
			
		    $media_url =  Mage::getBaseUrl().'media/uploads'.$filearray[0];
			$media_url = str_replace('index.php/', '', $media_url); 
			
			$file_name = explode("/",$filearray[0]);
			...
			$senderName = "Satish C";
			$senderEmail = "satish.visit@gmail.com";
			$companymail = "kushagra.daharwal@hotmail.com";
			$emailName = "BEM";
			$storeId = 1;
			//$uploadfilename = 'file.pdf';
			$uploadfilename =  $filename;
			$vars = array( 'order'        => $this,
                'billing'      => $this->getBillingAddress(),
                'payment_html' => $paymentBlockHtml );
			...
			$transactionalEmail->getMail()
                ->createAttachment(
				//file_get_contents(Mage::getBaseDir('var').DS.'docs'.DS.$uploadfilename),
                file_get_contents($media_url),				
		...
		$transactionalEmail->sendTransactional(2, $sender, $companymail, $emailName,$vars,$storeId);
